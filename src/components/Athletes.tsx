...
import { athletes, type Athlete } from '../data/mockData'
    ...
    <div style={{ animation: 'fadeIn 0.35s ease forwards' }}>
      <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-start', marginBottom: 24 }}>
        <div>
          ...
          <p style={{ color: '#6b7280', fontSize: 13, margin: '4px 0 0' }}>{athletes.length} спортсменов · {athletes.filter(a => a.status === 'active').length} активных</p>
        </div>
        <button style={{
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(auto-fill, minmax(300px, 1fr))', gap: 16 }}>
        {filtered.map(a => <AthleteCard key={a.id} athlete={a} onClick={() => setSelectedId(a.id)} />)}
        {filtered.length === 0 && (
    ...
    >
      ...
      <div style={{ display: 'flex', gap: 14, marginBottom: 14 }}>
        ...
        <div style={{ flex: 1 }}>
          ...
          <div style={{ fontSize: 12, color: '#6b7280', marginBottom: 4 }}>{athlete.birthDate.slice(0, 4)} · {athlete.gender === 'M' ? 'Муж' : 'Жен'}</div>
      ...
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8 }}>
        {[
          ...
          { l: 'Специал.', v: athlete.gender === 'F' ? 'Семиборье' : 'Десятиборье' },
        ].map(s => (
      ...
      <div style={{ marginTop: 12, paddingTop: 12, borderTop: '1px solid #1e2230', display: 'flex', justifyContent: 'space-between', alignItems: 'center' }}>
        <span style={{ fontSize: 12, color: '#6b7280' }}>{athlete.favoriteEvent}</span>
        <span style={{
          fontSize: 11,
